...
(defmethod handle-grain-event-sc-32ch ((g grain-event-32ch) timestamp &key)
  (unless (gethash (grain-32ch-sample-location g) *sc-buffer-directory*)
    (register-sample (grain-32ch-sample-location g)))
  ;; might save a hashtable access here ... later ...
  (let ((bufnum (gethash (grain-32ch-sample-location g) *sc-buffer-directory*)))    
    (if (> (event-reverb g) 0)             
	(osc:simple-bundle cm::*oscout* timestamp  
			   "/s_new"	    
			   "siiisisfsfsfsfsfsfsfsfsfsfsfsfsfsfsfsfsfsfsf"
			   "grain_32ch_rev" -1 0 1
			   "bufnum" bufnum
